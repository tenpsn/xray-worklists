...
### วิธีที่เร็วที่สุด — ใช้ `setup.bat` (Windows)

ที่ root ของโปรเจกต์มีสคริปต์ **`setup.bat`** ให้ใช้ตอนติดตั้งบนเครื่องใหม่ ทำให้ครบทุกขั้นตอนในคำสั่งเดียว เปิด Command Prompt (หรือดับเบิลคลิกไฟล์ก็ได้) ที่ root โปรเจกต์แล้วรัน:
...
1. เช็คว่ามี Docker / `docker compose` พร้อมใช้งานไหม
2. สร้าง `.env` และ `backend\.env` จากไฟล์ `.env.example` ให้ (ถ้ายังไม่มี — ถ้ามีอยู่แล้วจะไม่ทับ)
3. หยุดรอให้เปิดแก้ค่าใน env ให้ตรงกับเครื่องนี้ (ฐานข้อมูล HIS, API URL) ก่อนกด Enter ไปต่อ
4. รัน `docker compose build` แล้ว `docker compose up -d` ให้อัตโนมัติ
5. แสดงสถานะ container ตอนจบ

ถ้าอยากข้ามขั้นตอนหยุดรอแก้ env (เช่นไฟล์ env แก้ไว้ล่วงหน้าแล้ว) ใช้:

```bat
setup.bat -y
```

### หรือรันคำสั่ง Docker เองทีละขั้น
