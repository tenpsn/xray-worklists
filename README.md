...
- **Backend** (พอร์ต 4000, สำรองไว้เผื่ออนาคต 4001) — คุยกับ HIS DB และคุยกับเครื่อง X-ray
- **Frontend** (พอร์ต 3000, สำรองไว้เผื่ออนาคต 3001) — หน้าเว็บที่เราเปิดดูตารางผ่านเบราว์เซอร์
...
- **`DicomAlwaysAllowFind`** / **`DicomAlwaysAllowFindWorklist`** — ตั้งเป็น `false` ทั้งคู่ (ค่าแรกคุมภาพรวม ค่าที่สองเจาะจงเฉพาะการดึง Worklist) เพื่อความปลอดภัย เครื่องที่จะ query ได้ต้องลงทะเบียน AE Title ไว้ล่วงหน้าใน `DicomModalities` ก่อนเท่านั้น ไม่งั้นจะถูกปฏิเสธการเชื่อมต่อ
- **`DicomModalities`** — ลงทะเบียนเครื่อง X-ray ที่อนุญาตให้ query ได้ ระบุ AE Title, IP, Port ของเครื่อง (ต้องตรงกับที่กรอกไว้ในขั้นที่ 5) เช่น:
  ```json
  "DicomModalities": {
    "XRAY1": ["ORTHANC", "192.168.1.100", 104]
  }
  ```
  ชื่อ key (`XRAY1`) ตั้งเป็นอะไรก็ได้ ใช้แค่เป็นชื่ออ้างอิงภายใน ไม่กระทบการเชื่อมต่อ
